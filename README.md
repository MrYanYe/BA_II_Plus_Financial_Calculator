...
**Deliberately different — six changes:**
...
4. **Opening a panel does not steal focus.** Upstream focuses the matching field the moment a
   worksheet appears — press `8` then `N` and the caret lands in `N`. On a phone that scrolls the
   panel into view (animated, because `styles.css` sets `scroll-behavior: smooth`) and raises the
   virtual keyboard, so the calculator you were typing on disappears and every later keypress
   scrolls back to the field.

   Here the panel just opens. Tapping or clicking a field still focuses it and still raises the
   keyboard — that goes through the browser's own default action, not the method being
   suppressed, so the keyboard appears when you ask for it and not before.
...
6. **A finished calculation ends the entry.** Upstream keeps a result in the entry buffer, so
   `1 + 2 =` then `4` gives 34. On the device the `3.00` is a finished number and `4` starts a new
   one. Operators are the opposite case — they keep the value, because it is the left operand — so
   `1 + 2 =` then `* 3 =` is still 9.

   This applies after `=`, `√`, `x²`, `1/x`, `ln`, `%` and a `CPT` solve, and it is the same rule
   that governs `STO` and `RCL` above.
...
│   ├── ce_c_behavior.js        two-stage CE|C, like the real device
│   ├── panel_focus.js          stop a worksheet grabbing focus when it opens
│   └── entry_behavior.js       STO/RCL on the keypad, and when an entry ends
│
...
│   ├── verify_ce_c.py          two-stage CE|C
│   ├── verify_panel_focus.py   no panel steals focus when it opens
│   ├── verify_entry_behavior.py  STO/RCL, TVM recall, and fresh-entry rules
...
python tools/verify_panel_focus.py   # panels do not steal focus (local only, no network)
python tools/verify_entry_behavior.py # STO/RCL and entry rules (local only, no network)
...
| `STO` / `RCL` from keypad and panel (deliberately different) | ten registers, mid-expression and TVM recall, cancel paths |
| Fresh entry after `=` and the maths keys (deliberately different) | a digit starts a new calculation; an operator keeps the result |
...
4. **打开面板时不再抢占焦点。** 线上只要工作表一出现，就会把光标放进对应字段 —— 按 `8` 再按 `N`，
   光标就落进 `N`。在手机上这会滚动页面把面板带进视野（而且是动画，因为 `styles.css` 里设了
   `scroll-behavior: smooth`），同时弹出虚拟键盘，于是你正在按的计算器就不见了，之后每按一个键
   都会再滚回那个字段。

   这里改成面板只是打开。手动点按字段依然会聚焦、依然会弹出键盘 —— 那条路径走的是浏览器自身的
   默认行为，不经过被屏蔽的那个方法，所以键盘只在你主动要求时才出现。
...
6. **一次计算结束后，当前输入也随之结束。** 线上会把结果留在输入缓冲里，所以 `1 + 2 =` 之后按 `4`
   会得到 34。真机上那个 `3.00` 是一个已完成的数值，按 `4` 应当开始一个新数。运算符是相反的情况 ——
   它保留当前值，因为那是左操作数 —— 所以 `1 + 2 =` 之后 `* 3 =` 仍然是 9。

   这条规则适用于 `=`、`√`、`x²`、`1/x`、`ln`、`%` 以及 `CPT` 求解之后；上面 `STO` 与 `RCL` 用的
   也是同一条规则。
...
│   ├── ce_c_behavior.js        两段式 CE|C，与真机一致
│   ├── panel_focus.js          工作表面板打开时不再抢占焦点
│   └── entry_behavior.js       STO/RCL 键盘操作，以及何时结束当前输入
│
...
│   ├── verify_ce_c.py          两段式 CE|C
│   ├── verify_panel_focus.py   面板打开时不抢占焦点
│   ├── verify_entry_behavior.py  STO/RCL、TVM 调用、重新开始输入的规则
...
python tools/verify_panel_focus.py   # 面板不抢占焦点（纯本地，无需联网）
python tools/verify_entry_behavior.py # STO/RCL 与输入规则（纯本地，无需联网）
...
| 手动点按字段 | 仍然聚焦、仍然弹出键盘 |
| `STO` / `RCL` 键盘与面板两种操作（有意不同） | 十个寄存器、中途调用与 TVM 调用、取消路径正常 |
| `=` 与各数学键之后重新开始输入（有意不同） | 数字键开始新计算；运算符保留结果 |
