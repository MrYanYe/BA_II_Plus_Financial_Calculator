...
│
├── src/                        working set; builds into the file above
│   ├── page.html               calculator-only page, generated by the extractor
...
│   ├── extract_calculator.py   isolate the widget into src/
...
`src/page.html` is a normal page — open it directly in a browser while developing; it links its
siblings as plain files.
...
│   ├── extract_calculator.py   提取出计算器到 src/
...
`src/page.html` 是一个正常网页，开发时可直接用浏览器打开，它以普通文件方式引用同级资源。
