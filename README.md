...
## Fidelity — what matches, and the five things that do not
...
**Deliberately different — five changes:**
...
│   └── sto_rcl_behavior.js     STO/RCL on the keypad, usable mid-calculation
│
...
│   ├── verify_sto_rcl.py       STO/RCL from the keypad, including mid-expression
...
python tools/verify_sto_rcl.py       # STO/RCL from the keypad (local only, no network)
...
│   └── sto_rcl_behavior.js     STO/RCL 键盘操作，可在运算中途调用
│
...
│   ├── verify_sto_rcl.py       STO/RCL 键盘操作，含运算中途调用
...
python tools/verify_sto_rcl.py       # STO/RCL 键盘操作（纯本地，无需联网）
